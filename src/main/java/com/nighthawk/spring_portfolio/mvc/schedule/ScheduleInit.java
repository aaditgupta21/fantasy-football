...
public class ScheduleInit {
    ...
    // Inject repositories
    @Autowired
    ScheduleJpaRepository repository2;
    ...
    CommandLineRunner run2() { // The run() method will be executed after the application starts
        return args -> {
            ...
            for (String fact : factsArray) {
                List<Schedule> test = repository2.findByFactIgnoreCase(fact); // JPA lookup
                if (test.size() == 0)
                    repository2.save(new Schedule(null, fact, 0, 0)); // JPA save
            }
